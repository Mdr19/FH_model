        ...
        function obtain_MPC_model(obj,N_,p_,Tp,h)
            if ~isempty(obj.current_model)
                ...
                if size(obj.current_model)==1
                    ...
                    Bc=obj.current_model.B(:,2:end);
                    Cc=zeros(1,size(Ac,1));
                    Cc(1,size(Ac,1))=1;
                       
                    X0=obj.current_model.A*obj.current_initial_state'+...
                    obj.current_model.B(:,2:end)*(obj.signals_intervals(end).original_signals(2:2+size(obj.current_model.B,2)-2,end)-...
                    obj.ident_models(obj.current_model_nr).offset_value(2:2+size(obj.current_model.B,2)-2)');
                    ...
                    X0=obj.current_model(2).A*obj.current_initial_state(2,:)'+...
                    obj.current_model(2).B*(obj.signals_intervals(end).original_signals(2:2+size(obj.current_model(2).B,2)-1,end)-...
                    obj.ident_models(obj.current_model_nr).offset_value(2:2+size(obj.current_model(2).B,2)-1)');
                    X0=[X0; obj.signals_intervals(end).original_signals(3,end)-...
                        obj.ident_models(obj.current_model_nr).offset_value(end)];
