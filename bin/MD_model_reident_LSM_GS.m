function [interval_type, best_model] = MD_model_reident_LSM_GS(current_model,inputs_to_ident,sys_input,sys_output,initial_state,plot_nr)
...
for i=1:ident_inputs_nr
    C=zeros(1,m-1);
    C(end)=1;
    state_space=ss(current_model(i).A,current_model(i).B,C,[0]);
...
if MD_constant_values.sum_sqr_difference
    initial_error=sumsqr(sys_output'-pv_curr);
else
    initial_error=sum(abs(sys_output'-pv_curr));
end
...
if initial_error>MD_constant_values.model_change_threshold
    ...
    ni_min=MD_MFM_model_ident_out_LSM(ident_inputs_nr,sys_ident_input,sys_output);
...
current_model=input_model;
    ...
    plot(sys_output);
    plot(sum(y_sim));
    legend('initial','real output','identified');
    ...
    for i=1:length(best_model)
        current_model(i).A=input_model(i).A;
        current_model(i).B=input_model(i).B;
