...
for i=1:inputs_nr
    ...
    models(i).C(end)=1;
    models(i).D=[0];
    models(i).vector=[ni_min_(1:m-1); ni_min_(m-1+i)];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=1:length(models)
    ...
    [test_model(k).A]=obtain_SS_SISO_model(test_model(k).vector,m);
    %test_model(k).A
    eig(test_model(k).A)
end
...
if sum(isnan(p)==0)
    ...
    for i=1:length(models)
        ...
        [models(1).A,models(1).B]=obtain_SS_SISO_model(models(1).vector,m);
    end
else
...
for nr=1:sim_max_iters
    ...
    if sum(isnan(p)==0)
        ...
        for i=1:length(models)
            ...
            models(i).C(end)=1;
            models(i).D=[0];
            [models(i).A,models(i).B]=obtain_SS_SISO_model(models(i).vector,m);
        end
    else
